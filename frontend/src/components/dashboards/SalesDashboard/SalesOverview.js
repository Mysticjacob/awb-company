...
import "./SalesDashboard.css"; // Import CSS styles
...
const SalesDashboard = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get("https://cloud-computing-d7av.vercel.app/api/products");
  ...
  const fetchSalesData = async () => {
    try {
      const response = await axios.get("https://cloud-computing-d7av.vercel.app/api/sales/overview", {
  ...
  const handleAddProduct = async () => {
    try {
      setLoading(true);
      await axios.post("https://cloud-computing-d7av.vercel.app/api/products", newProduct, {
        headers: { Authorization: `Bearer ${token}` },
      });
      alert("Product added!");
  ...
  const handleDeleteProduct = async (productId) => {
    ...
    try {
      await axios.delete(`https://cloud-computing-d7av.vercel.app/api/products/${productId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
