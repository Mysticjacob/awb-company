...
const SalesCharts = () => {
  ...
  const chartRef = useRef(null); // ✅ Correctly initialize chart reference

  useEffect(() => {
    const fetchSalesRecords = async () => {
      try {
        const response = await axios.get("https://cloud-computing-ecru.vercel.app/api/sales");
        ...
        generateChartData(response.data); // ✅ Ensure data is correctly processed for the chart
      } catch (err) {
    ...
    return () => {
      if (chartRef.current && chartRef.current.destroy) {
        chartRef.current.destroy(); // ✅ Properly destroy existing chart instances
      }
    };
  }, []);

  // ✅ Generate Chart Data Correctly
  const generateChartData = (salesRecords) => {
    ...
    salesRecords.forEach((record) => {
      ...
      const dateString = purchaseDate.toLocaleDateString("en-GB"); // Format: DD/MM/YYYY
